...
fn no_alarm_for_normal_dc_voltage() {
    let mut m = make_measurement(MeasurementMode::DcVoltage, Some(5.0));
    let state = AlertEvaluator::evaluate(&mut m, &default_config(), AlarmState::None);
    assert_eq!(state, AlarmState::None);
}

#[test]
fn high_alarm_triggered() {
    let mut m = make_measurement(MeasurementMode::DcVoltage, Some(13.0));
    let state = AlertEvaluator::evaluate(&mut m, &default_config(), AlarmState::None);
...
fn low_alarm_triggered() {
    let mut m = make_measurement(MeasurementMode::DcVoltage, Some(2.0));
    let state = AlertEvaluator::evaluate(&mut m, &default_config(), AlarmState::None);
...
fn high_alarm_hysteresis_holds() {
    ...
    // Aditive hysteresis: clear_threshold = 12.0 - 12.0 * 0.01 = 11.88
    let clear_threshold = 12.0 - 12.0 * 0.01;
    let mut m = make_measurement(MeasurementMode::DcVoltage, Some(clear_threshold + 0.01));
    let state = AlertEvaluator::evaluate(&mut m, &config, AlarmState::HighAlarm);
...
fn high_alarm_clears_below_hysteresis() {
    let config = default_config();
    let clear_threshold = 12.0 - 12.0 * 0.01;
    let mut m = make_measurement(MeasurementMode::DcVoltage, Some(clear_threshold - 0.1));
    let state = AlertEvaluator::evaluate(&mut m, &config, AlarmState::HighAlarm);
    assert_eq!(state, AlarmState::None);
}
...
fn no_short_above_threshold() {
    let mut m = make_measurement(MeasurementMode::Continuity, Some(5.0));
    let state = AlertEvaluator::evaluate(&mut m, &default_config(), AlarmState::None);
    assert_eq!(state, AlarmState::None);
}
...
fn no_alarm_for_non_dcv_mode() {
    let mut m = make_measurement(MeasurementMode::AcVoltage, Some(300.0));
    let state = AlertEvaluator::evaluate(&mut m, &default_config(), AlarmState::None);
    assert_eq!(state, AlarmState::None);
}

#[test]
fn evaluate_stamps_short_flag() {
    let mut m = make_measurement(MeasurementMode::Continuity, Some(0.5));
    let config = default_config();
    let state = AlertEvaluator::evaluate(&mut m, &config, AlarmState::None);
    assert_eq!(state, AlarmState::Short);
    assert!(m.is_short);
}

#[test]
fn evaluate_does_not_stamp_when_above_threshold() {
    let mut m = make_measurement(MeasurementMode::Continuity, Some(5.0));
    let config = default_config();
    let state = AlertEvaluator::evaluate(&mut m, &config, AlarmState::None);
    assert_eq!(state, AlarmState::None);
