...
use readout_core::types::{
    Command, DB_REFERENCE_VALUES, DbReference, MathFunction, MultimeterCommand, MultimeterRange,
...
pub fn show(
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        ui.add_enabled_ui(range_enabled, |ui| {
            ui.horizontal(|ui| {
                if ui.button(egui::RichText::new("◀").size(14.0)).clicked() {
                    send_command(
                        command_tx,
                        MultimeterCommand::SetRange(MultimeterRange::Manual(1)),
                    );
                }
                ui.label(
                    egui::RichText::new(if state.meter_range_label.is_empty() {
                        "---"
                    } else {
                        &state.meter_range_label
                    })
                    .size(14.0)
                ...
                if ui.button(egui::RichText::new("▶").size(14.0)).clicked() {
                    send_command(
                        command_tx,
                        MultimeterCommand::SetRange(MultimeterRange::Manual(7)),
                    );
                }
            });
        });
        ...
        if state.meter_mode == MeasurementMode::Temperature {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.selectable_label(false, "KITS90").clicked() {
                    send_command(
                        command_tx,
                        MultimeterCommand::SetTempSensorType(TempSensorType::Kits90),
                    );
                }
                if ui.selectable_label(false, "PT100").clicked() {
                    send_command(
                        command_tx,
                        MultimeterCommand::SetTempSensorType(TempSensorType::Pt100),
                    );
                }
            });
            ui.horizontal(|ui| {
                ...
                if ui.selectable_label(false, "°C").clicked() {
                    send_command(
                        command_tx,
                        MultimeterCommand::SetTempUnit(TempUnit::Celsius),
                    );
                }
                if ui.selectable_label(false, "°F").clicked() {
                    send_command(
                        command_tx,
                        MultimeterCommand::SetTempUnit(TempUnit::Fahrenheit),
                    );
                }
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .selectable_label(active == Some(MathFunction::Average), "MIN/MAX")
                .clicked()
            {
                if active == Some(MathFunction::Average) {
                    ...
                    send_command(
                        command_tx,
                        MultimeterCommand::StartMath(MathFunction::Average),
                    );
                }
            }
            if ui
                .selectable_label(active == Some(MathFunction::Null), "REL")
                .clicked()
            {
            ...
            if ui
                .selectable_label(active == Some(MathFunction::Db), "dB")
                .clicked()
            {
            ...
            if ui
                .selectable_label(active == Some(MathFunction::Dbm), "dBm")
                .clicked()
            {
                ...
                egui::ComboBox::from_id_salt("db_ref")
                    ...
                    .show_ui(ui, |ui| {
                        for &ohms in DB_REFERENCE_VALUES {
                            if ui
                                .selectable_label(false, format!("{ohms} \u{2126}"))
                                .clicked()
                            {
                                send_command(
                                    command_tx,
                                    MultimeterCommand::SetDbReference(DbReference::Ohms(ohms)),
                                );
                            }
                        }
                    });
        ...
        if state.meter_math_function == Some(MathFunction::Average) {
            if let Some(stats) = &state.meter_math_stats {
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(format!("Min: {:.4}", stats.min))
                            .size(10.0)
                            .color(sec),
                    );
                    ui.label(
                        egui::RichText::new(format!("Max: {:.4}", stats.max))
                            .size(10.0)
                            .color(sec),
                    );
                    ui.label(
                        egui::RichText::new(format!("Avg: {:.4}", stats.avg))
                            .size(10.0)
                            .color(sec),
                    );
                });
            ...
            if last.is_none_or(|t| now.duration_since(t).as_secs_f32() >= 1.0) {
        ...
        ui.horizontal(|ui| {
            if ui
                .button(egui::RichText::new(format!("{} Lock Panel", icons::LOCK_KEY)).size(11.0))
                .clicked()
            {
                send_command(command_tx, MultimeterCommand::SetRemoteMode(true));
            }
            if ui
                .button(egui::RichText::new(format!("{} Unlock", icons::LOCK_KEY_OPEN)).size(11.0))
                .clicked()
            {
        ...
        if ui
            .button(
                egui::RichText::new(format!("{} Reset Device", icons::ARROW_COUNTER_CLOCKWISE))
                    .size(11.0),
            )
            .clicked()
        {
        ...
        ui.add_enabled_ui(true, |ui| {
            ...
            ui.add_space(2.0);
            ui.horizontal(|ui| {
                let pc_icon = if pc_beep_enabled {
                    icons::SPEAKER_HIGH
                } else {
                    icons::SPEAKER_SLASH
                };
                if ui
                    .selectable_label(
                        pc_beep_enabled,
                        egui::RichText::new(format!("{pc_icon} PC beep")).size(11.0),
                    )
                    .clicked()
                {
                ...
                let meter_icon = if meter_beep_enabled {
                    icons::BELL_RINGING
                } else {
                    icons::BELL_SLASH
                };
                if ui
                    .selectable_label(
                        meter_beep_enabled,
                        egui::RichText::new(format!("{meter_icon} Meter beep")).size(11.0),
                    )
                    .clicked()
                {
